...
def shop_trip() -> None:
    ...
    def cost_of_km(customer: Customer) -> int | float:
        return (customer.car["fuel_consumption"] / 100) * data["FUEL_PRICE"]

    def cost_by_distance(customer: Customer, shop: Shop) -> int | float:
        return (math.dist(
            customer.location, shop.location
        ) * 2) * cost_of_km(customer)
    ...
    def total_price(customer: Customer, shop: Shop) -> int | float:
    ...
    for customer_data in data["customers"]:
        ...
        for shop_data in data["shops"]:
            ...
            total_cost = total_price(customer, shop)
